...
- **Location:** `src/linux/init/config.cpp`
- **Attacker:** Any local unprivileged user in the WSL instance when `[boot] systemd = true` is set in `/etc/wsl.conf`
- **Controlled input:** `LX_INIT_CREATE_LOGIN_SESSION` username and UID/GID sent over the world-writable interop socket
- **Attack path:** Handler accepts `LxInitMessageCreateLoginSession` without peer-credential check and calls `CreateLoginSession`, which `forkpty` then `execl("/bin/login", "/bin/login", "-f", Username, nullptr)` to skip PAM authentication and activate `user@<Uid>.service`
- **Impact:** Unauthenticated activation of systemd login sessions for any user including root, starting privileged user services
